...
if (inc.value.length == 0 || fim.value.length == 0 || pas.value.length == 0) {
    alert("[ERRO] Faltam Dados!")
    } else if (inc.value >= 115 || fim.value >= 115 || inc.value <= -115 || fim.value <= -115) {
        alert("[ERRO] O Valor não pode ultrapassar 114 números!!")
    } else if (pas.value <= -1) {
        alert("[ERRO] O passo não pode ser negativo")
    } else {
        ...
        if (p == 0) {
            ...
            p = 1
        } else if (i >= 36 || f >= 36 || i <= -36 || f <= -36 && p == 1) {
            res.style.fontSize = "12pt";
        } else if (i >= 111 || f >= 111 || i <= -111 || f <= -111 && p == 1) {
